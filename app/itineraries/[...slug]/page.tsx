...
import { GET_ITINERARY_BY_PATH } from '@/lib/queries'
import { DrupalItinerary } from '@/lib/types'
...
interface ItineraryByPathData {
  route: {
    entity: DrupalItinerary
  } | null
}

async function getItinerary(path: string): Promise<DrupalItinerary | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<ItineraryByPathData>({
      query: GET_ITINERARY_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/itineraries/${slug.join('/')}`
  const item = await getItinerary(path)

  if (!item) {
    return { title: 'Itinerary Not Found | Tourism Board' }
  }

  return {
    title: `${item.title} | Tourism Board`,
